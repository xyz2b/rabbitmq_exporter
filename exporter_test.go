...
func TestQueueState(t *testing.T) {
	...
	// queue
	expectSubstring(t, body, `rabbitmq_queue_state{durable="true",policy="",queue="myQueue1",state="flow",vhost="/"} 1`)
	expectSubstring(t, body, `rabbitmq_queue_state{durable="true",policy="",queue="myQueue3",state="running",vhost="/"} 1`)
	expectSubstring(t, body, `rabbitmq_queue_state{durable="true",policy="ha-2",queue="myQueue2",state="running",vhost="/"} 1`)

	// connections
	expectSubstring(t, body, `rabbitmq_connection_status{node="rabbit@rmq-cluster-node-04",peer_host="172.31.0.130",state="running",user="rmq_oms",vhost="/"} 2`)
}
