...
func (e exporterConnections) Collect(ctx context.Context, ch chan<- prometheus.Metric) error {
	...
	for key, gauge := range e.connectionMetricsG {
		for _, connD := range rabbitConnectionResponses {
			if value, ok := connD.metrics[key]; ok {
				...
				gauge.WithLabelValues(cluster, host, subsystemName, subsystemID, connD.labels["vhost"], connD.labels["node"], connD.labels["peer_host"], connD.labels["user"], self).Add(value)
			}
		}
	}
	...
	for _, connD := range rabbitConnectionResponses {
		...
		e.stateMetric.WithLabelValues(cluster, host, subsystemName, subsystemID, connD.labels["vhost"], connD.labels["node"], connD.labels["peer_host"], connD.labels["user"], connD.labels["state"], self).Add(1)
	}
